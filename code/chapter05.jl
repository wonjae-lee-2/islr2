...
# ╔═╡ 975dff4a-d81d-11ec-03a7-77c112bb18f9
using CSV, MLJ, Plots

# ╔═╡ 75d2b35b-f947-4de5-a939-78366b89e717
import DataFrames, MLJLinearModels

# ╔═╡ e7e566de-3c75-41ab-91a1-6c84f7f72e04
datafolder = expanduser("~/github/islr2/data")

# ╔═╡ 96538abe-8504-43eb-9282-c9dcd0f2e444
dfauto_raw = CSV.read(joinpath(datafolder, "Auto.csv"), DataFrames.DataFrame)

# ╔═╡ 1aa5fc2b-ff3f-4b77-bb17-39fd575b3cd9
dfauto = dfauto_raw |>
	x -> DataFrames.subset(x, :horsepower => a -> a .!= "?") |>
	x -> DataFrames.transform(x, :horsepower => DataFrames.ByRow(a -> parse(Int, a)) => :horsepower)

# ╔═╡ bbd893a0-e9eb-4f20-8e26-8b2bfbb0aebd
schema(dfauto)

# ╔═╡ c0109fe2-d054-4a9a-bee5-13b9208c436d
y = dfauto[:, :mpg]

# ╔═╡ a951c1c7-e753-41c0-a896-3063829f5006
X =	dfauto[:, [:horsepower]] |> 
	x -> coerce(x, :horsepower => Continuous)

# ╔═╡ 5846f0a3-7031-4018-af6c-a6890bb401b5
models(matching(X, y))

# ╔═╡ 703336ea-251c-40c6-b0e0-401e1386679d
LinearRegressor = @load LinearRegressor pkg = MLJLinearModels

# ╔═╡ 081ced88-ead7-48cf-b5a6-7d0465fb5db9
linear = LinearRegressor()

# ╔═╡ fe9b30c2-ea6e-4f27-9b50-688b960b3a86
mach_linear1 = machine(linear, X, y)

# ╔═╡ de85f7d5-32e1-4054-915f-4ede898db823
evaluate!(mach_linear1, resampling = Holdout(fraction_train = 0.5, rng = 1), measure = rmse)

# ╔═╡ fd27eefb-84be-4ef3-a974-225c11f21523
X2 = DataFrames.transform(X, :horsepower => DataFrames.ByRow(x -> x^2) => :horsepower2)

# ╔═╡ f0eb1a2d-9538-4159-bc5c-b4f621f93ed4
mach_linear2 = machine(linear, X2, y)

# ╔═╡ fd6c8b05-6512-40ae-a9a4-2b67f84366b7
evaluate!(mach_linear2, resampling = Holdout(; fraction_train = 0.5, rng = 2), measure = rmse)

# ╔═╡ aa35645d-98d8-4a27-9852-c341dc211e88
X3 = DataFrames.transform(X2, :horsepower => DataFrames.ByRow(x -> x^3) => :horsepower3)

# ╔═╡ 4a4f5950-1cf2-4e04-aa55-fe5f7e9881bc
mach_linear3 = machine(linear, X3, y)

# ╔═╡ 47e64892-93b1-454e-be88-1932df3c413e
evaluate!(mach_linear3, resampling = Holdout(; fraction_train = 0.5, rng = 3), measure = rmse)

# ╔═╡ 00be707a-478d-4da7-a653-8907bb777178
function poly(df, degree)
	col = df[:, 1]
	result = DataFrames.DataFrame([col.^x for x in 1:degree], :auto)
	return result
end

# ╔═╡ 68f38971-e095-4866-8bdb-2de564924926
function loocv(df, degree)
	result = evaluate(linear, poly(df, degree), y, resampling = CV(; nfolds = DataFrames.nrow(df), rng = 4), measure = rmse)
	return result.measurement
end

# ╔═╡ 7ac3a4f0-b052-4454-bf6a-9fd59e68c2f2
loocv(X, 1)

# ╔═╡ 25789391-7eed-462a-9b88-e791aaf49046
loocv
...
git-tree-sha1 = "773f766b92c8c5500698899c1d5c568ad8c2be00"
...
git-tree-sha1 = "ba8e9b58638d1a4214cf3a556ec8999642f01bf6"
...
git-tree-sha1 = "f18f907323cf5001c52f57a5684fe068383e2494"
